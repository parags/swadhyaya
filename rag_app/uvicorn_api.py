...
GROQ_API_KEY = os.getenv(
    "GROQ_API_KEY")  # Ensure this is set in your environment variables
if not GROQ_API_KEY:
    raise ValueError(
...
embedding_model = SentenceTransformer(
    "all-MiniLM-L6-v2")  # Lightweight & effective
...
def query_chromadb(query_text: str, top_k: int = 3):
    ...
    for i, doc in enumerate(results["documents"][0]):
        ...
        response.append({"filename": metadata["filename"], "text": doc})
...
@app.get("/search")
def search(word: str = Query(..., description="Word to search for"),
           userContext: str = Query(
               ..., description="Additional voice context from the user"),
           top_k: int = Query(3,
